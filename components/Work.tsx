...
export default function Work() {
    ...
    return (
        <div
            className="relative min-h-screen w-full bg-slate-300 dark:bg-gradient-to-b dark:from-slate-800 to-black py-[70px]">
            {selectedImage > -1 && (
                <Modal isOpen={open} setVisibility={setVisibility}>
                    <div
                        className="w-full h-full lg:container lg:max-w-[60vw] max-h-[80vh]  bg-white dark:bg-black bg-opacity-70 dark:bg-opacity-70 backdrop-blur-md  flex flex-col">
                ...
                <div className="lg:container h-full  w-full flex items-center overflow-auto ">
                    <div className="grid grid-flow-row grid-cols-1 md:grid-cols-3 lg:grid-cols-4   gap-4  mx-auto ">
                        {createArray(57).map((item) => (
                            ...
                            >
                                <div
                                    className="relative w-80   h-40 rounded-lg shadow-xl hover:shadow-2xl bg-black overflow-hidden">
                                    <Image
                                        onClick={() => {
